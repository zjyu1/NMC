...
FLEXFUNC flex_read_limit_status_rtn(BOARD, u16 limitType, u8 *forwardLimitStatus, u8 *reverseLimitStatus);
...
FLEXFUNC flex_read_dac_limit_status_rtn(BOARD, u8 *positiveStatus, u8 *negativeStatus);
...
// Wait on condition (On Brd program)
...
FLEXFUNC flex_read_adc_rtn (BOARD, u8 ADC, i16 *ADCValue);
...
FLEXFUNC flex_read_traj_data_rtn(BOARD, u8 device, long *retdata);
...
//OBSOLETE
//READ_RUNST_CMD, Read Run/Stop Status
FLEXFUNC flex_read_rs_status_rtn(BOARD, u8 device, u16 *status);

//READ_MOFF_CMD, Read Motor Off Status
FLEXFUNC flex_read_motoff_status(BOARD, u8 device, u8 retvect);

//READ_MOFF_CMD, Read Motor Off Status
FLEXFUNC flex_read_motoff_status_rtn(BOARD, u8 device, u16 *status);
...
//OBSOLETE
//READ_DACLIM_CMD, Read DAC Limit Status
FLEXFUNC flex_read_dac_status_rtn(BOARD, u8 device, u16 *status);


//Load all Control Loop Parameters
FLEXFUNC flex_load_loop_params(BOARD, u8 axis, PID *PIDValues, u8 inputVector);
...
FLEXFUNC flex_read_boot_ver_rtn(BOARD, u32 *bootver);

//Read Firmware Version Number
FLEXFUNC flex_read_firm_ver_rtn(BOARD, u32 *firmver);

//Read DSP Version Number
FLEXFUNC flex_read_dsp_ver_rtn(BOARD, u32 *dspver);

//Read FPGA Version Number
FLEXFUNC flex_read_fpga_ver_rtn(BOARD, u32 *fpgaver);
...
//Read Home Input Status
FLEXFUNC flex_read_home_status_rtn(BOARD, u8 device, u16 *homestatus);

//Read Velocity Threshold Status
FLEXFUNC flex_read_vel_status(BOARD, u8 device, u8 retvect);
//Read Velocity Threshold Status
FLEXFUNC flex_read_vel_status_rtn(BOARD, u8 device, u16 *status);
...
//Read Velocity
FLEXFUNC flex_read_vel_rtn(BOARD, u8 axisOrVectorSpace, i32 *velocity);
...
//Read Error Message
FLEXFUNC flex_read_err_msg_rtn(BOARD, u16 *commandID, u16 *resourceID, u16 *errorCode);
...
//Read Unfiltered Velocity
FLEXFUNC flex_read_raw_vel_rtn(BOARD, u8 axisorvectorspace, i32 *rawvel);
